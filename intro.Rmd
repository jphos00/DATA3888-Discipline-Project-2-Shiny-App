...
pred = predict(M1, newdata=dat, type="response")
```
...
## How do I fit a logistic LASSO regression model in R?

```{r, eval = F}
M2 = glmnet::glmnet(x=as.matrix(X), y = y, family="binomial", lambda=lambda)

pred = predict(M2, type="response", newx=as.matrix(X))
```
